...
from keras.layers import Input, Dense, Reshape, concatenate, AveragePooling2D
...
class DCGanV2(object):
    ...
    def fit_another_model(self, model_dir_path, image_label_pairs, epochs=None, batch_size=None, snapshot_dir_path=None,
        ...
        for epoch in range(epochs):
            ...
            for batch_index in range(batch_count):
                ...
                generated_images = self.img_generator.predict(text_batch, verbose=0)
                ...
                self.img_discriminator.trainable = True
                d_loss = self.img_discriminator.train_on_batch([np.concatenate((image_batch, generated_images)),
                ...
                if (epoch * batch_size + batch_index) % 10 == 9:
                    self.img_generator.save_weights(DCGanV2.get_weight_file_path(model_dir_path, 'generator'), True)
                    self.img_discriminator.save_weights(DCGanV2.get_weight_file_path(model_dir_path, 'discriminator'), True)
